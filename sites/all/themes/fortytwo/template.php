...
function fortytwo_preprocess_html(&$variables, $hook) {
  ...
  // Set edit tabs.
  if (theme_get_setting('ft_layout_tabs') === 1) {
    drupal_add_js(fortytwo_get_staticpath() . 'js/plugins/collapsibleTabs.js', [
      'type'       => 'file',
      'group'      => JS_THEME,
      'preprocess' => TRUE,
      'cache'      => TRUE,
    ...
    $variables['classes_array'][] = 'ft-tabs';
  }
  ...
  $ft_layout_login = theme_get_setting('ft_layout_login');
  if (!empty($ft_layout_login) && empty($user->uid)) {
    $variables['classes_array'][] = 'ft-layout-login';
  }
}
 ...
 */
function fortytwo_preprocess_page(&$variables) {
 ...
 *
 * Overrides collapse.js with our own collapse.js functionality. The
 * implementation from Drupal has a small overflow bug when a fieldset
 * is collapsed and re-opened.
    ...
    switch ($form_id) {
      ...
      case 'user_register_form':
        ...
        $form['account']['name']['#attributes'] = [
          'placeholder' => $form['account']['name']['#title'],
        ];
